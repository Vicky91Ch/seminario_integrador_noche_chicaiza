...
const VENDEDORES: Vendedor[] = [
  { id: 1, name: 'Carlos Mendoza',   department: 'Ventas',      salary: 4500, active: true  },
  { id: 2, name: 'Ana Torres',       department: 'Taller',      salary: 3800, active: true  },
  { id: 3, name: 'Luis Ramírez',     department: 'Administración', salary: 5200, active: false },
  { id: 4, name: 'María García',     department: 'Ventas',      salary: 4200, active: true  },
  { id: 5, name: 'Pedro Sánchez',    department: 'Taller',      salary: 3500, active: true  },
  { id: 6, name: 'Sofía López',      department: 'Administración', salary: 4800, active: false },
]

const VendedorRow = memo(function VendedorRow({
  emp,
  onToggle,
  onRaise,
  onRemove,
...
export default function FilterTable() {
  ...
  const departments = useMemo(
    () => ['Todos', ...new Set(VENDEDORES.map(e => e.department))],
    []
  )
  ...
  const visible = useMemo(
    ...
    [vendedores, deptFilter, showInactive]
  )

  const handleToggle = useCallback((id: number) => {
    setVendedores(prev => prev.map(e => e.id === id ? { ...e, active: !e.active } : e))
  }, [])

  const handleRaise = useCallback((id: number, amount: number) => {
    setVendedores(prev => prev.map(e => e.id === id ? { ...e, salary: e.salary + amount } : e))
  }, [])

  const handleRemove = useCallback((id: number) => {
    setVendedores(prev => prev.filter(e => e.id !== id))
  }, [])
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', maxWidth: 700, margin: '0 auto', padding: 24 }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Vendedores de Motos</h2>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 13, color: '#888', alignSelf: 'center' }}>
          {visible.length} vendedor{visible.length !== 1 ? 'es' : ''} · Salario total: ${totalSalary.toLocaleString()}
        </span>
      </div>
